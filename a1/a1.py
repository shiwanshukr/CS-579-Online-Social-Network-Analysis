...
def bfs(graph, root, max_depth):
  ...
  node2num_paths = {}
  node2parents = {}
  node2distances[root] = q
  node2num_paths[root] = q + 1
  deq = deque()
  deq.append(root)
  seen = set([root])
  while deq:
    ...
    for neighbor in graph.neighbors(n):
      if neighbor not in seen:
        seen.add(neighbor)
        deq.append(neighbor)
        node2parents[neighbor] = [n]
        node2distances[neighbor] = q + 1
        node2num_paths[neighbor] = node2num_paths[n]
      else:
        current_n = q + 1
        last_n = node2distances[neighbor]
        if(current_n == last_n):
          node2num_paths[neighbor] = node2num_paths[n] + node2num_paths[neighbor]
          node2parents[neighbor].append(n)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    nodes_maxdist = [k for k,v in node2distances.items() if v==max_distance]
    node_weight = {k:1 for k in node2distances.keys()}
    edges = []
    for key in sorted(node2distances, key=node2distances.get,reverse=True):
        if(key != root):
            for parent in node2parents[key]:
                node_weight[parent] += (node_weight[key]/len(node2parents[key]))
                if(key > parent):
                    edges.append((parent, key))
                else:
                    edges.append((key, parent))

    result = defaultdict()

    for edge in edges:
        if( root in edge):
            root_index = edge.index(root)
            if(root_index > 0):
                result[edge] = (node_weight[edge[0]]/len(node2parents[edge[0]]))
            else:
                result[edge] = (node_weight[edge[1]]/len(node2parents[edge[1]]))
        elif(edge[0] in node2parents[edge[1]]):
            result[edge] = (node_weight[edge[1]]/len(node2parents[edge[1]]))
        else:
            result[edge] = (node_weight[edge[0]]/len(node2parents[edge[0]]))
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in nodes:
        ...
        short_fract = bottom_up(node, node2distances, node2num_paths, node2parents)
        for edge in short_fract:
            result[edge] += short_fract[edge]

    for edge in result:
        result[edge] = (result[edge]/2.0)
...
def partition_girvan_newman(graph, max_depth):
    ...
    indexV=0
    result=sorted(betweenness.items(),key=lambda x: (-x[1], x[0]))
    component = get_components(graph_copy)
    while True:
        graph_copy.remove_edge(*(result[indexV][0]))
        component = get_components(graph_copy)
        if len(component) > 1:
            break
        elif len(component) == 1:
            indexV += 1
    component = sorted(component, key =lambda x: sorted(x.nodes())[0])
    return component
...
def volume(nodes, graph):
    ...
    ###TODO
    pass
    all_the_combinations_of_edges = graph.edges()
    count =0
    for edges in all_the_combinations_of_edges:
      if edges[0] in nodes or edges[1] in nodes:
        count+=1
    return count
...
def cut(S, T, graph):
    ...
    pass
    cut_count = 0
    edges_tup = graph.edges()
    for t in edges_tup:
        if ((t[0] in S) and (t[1] in T)):
            cut_count+= 1
        elif((t[0] in T) and (t[1] in S)):
            cut_count+=1
...
def norm_cut(S, T, graph):
    ...
    """
    ###TODO
    cutgraph = cut(S, T, graph)
    return ((float(cutgraph) / volume(S, graph)) + (float(cutgraph) / volume(T, graph)))
    pass
...
def brute_force_norm_cut(graph, max_size):
    ...
    for item in range(1,max_size+1):
        for firstCut in combinations(graph.edges(), item):
            graph_copy.remove_edges_from(firstCut)
            num_components = get_components(graph_copy)     
            if len(num_components) == 2:
                normalCut = norm_cut(num_components[0], num_components[1], graph)
                list_of_edges.append((normalCut,list(firstCut)))
            graph_copy.add_edges_from(firstCut)   
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        ...
        if(len(components)>1):
            normalCut = norm_cut(components[0], components[1], graph)
            Max_cut.append((depth,normalCut))
    return Max_cut
    pass
...
def make_training_graph(graph, test_node, n):
    ...
    ###TODO
    pass
    neighbors_of_n = sorted(graph.neighbors(test_node),key = lambda x:x[0])
    graph_copy = graph.copy()
    i = 0
    while(i<n):
        graph_copy.remove_edge(test_node,neighbors_of_n[i])
        i+=1
...
def jaccard(graph, node, k):
    ...
    for n in graph_nodes:
        if(n not in graph.neighbors(node)):
            node_neighbors2 = set(graph.neighbors(n))
            jc = (len(node_neighbors & node_neighbors2))/(len(node_neighbors | node_neighbors2 ))
            edge_score_list.append(((node,n),jc))

    sorted_list = sorted(edge_score_list, key=lambda x: (-x[1], x[0][1]))
    sorted_list.pop(0)
...
def evaluate(predicted_edges, graph):
    ...
    # sorted_predicted_edges = set(sorted(predicted_edges,key= lambda x:x[1]))
    # graph_sorted_edges = set(sorted(graph.edges(),key=lambda  x:x[1]))
    #
    # s1 = frozenset(sorted_predicted_edges)
    # s2 = frozenset(graph_sorted_edges)
    #
    # appeared = len(s1.intersection(s2))
    # # appeared = len(sorted_predicted_edges & graph_sorted_edges)
